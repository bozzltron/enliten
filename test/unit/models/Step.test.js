...
describe('Step Model', function() {
    ...
    it('.destroy() should fix ordering gaps when a middle items is deleted', function(done) {
    ...
    it('.destroy() should fix ordering gaps when the first item is deleted', function(done) {

        Path
            .create({name:'delete front test', user:'boz'})
            .then(function(path){
                return [path, Step.create([
                    {name:'step 1', order: 1, path:path.id, url: "1"},
                    {name:'step 2', order: 2, path:path.id, url: "2"},
            ...
            .spread(function(path, steps){
                return [path, steps[1], steps[2], Step.destroy({id:steps[0].id})];
            })
            .spread(function(path, step2, step3){
                return [path, step2, step3, Path.findOne({id:path.id}).populate('steps')];
            })
            .spread(function(path, step2, step3, pathResult){
